...
- `docker_health_alerts/docker_source.py` with a narrow Docker SDK adapter,
  container inspection snapshots, healthcheck detection, health log extraction,
  Docker event filters, and health event parsing.
- `docker_health_alerts/reconciliation.py` with startup reconciliation over a
  source protocol.
...
- Docker SDK objects are isolated in `docker_health_alerts/docker_source.py`.
